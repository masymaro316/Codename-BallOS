...
また、開発者の方々にこの全ての機能を開発するよう求めているわけではありません。開発者の方々のペースで開発していただければ結構です。
これはまだ計画の段階で、あまりにも過大なことは自覚しております。
# 計画、決定事項
・カーネル
　カーネルは、モノシリック、マイクロ、ハイブリットの三つのカーネルの版を用意する。(並行開発が困難な場合はハイブリッドカーネルに統合)
・開発期間など、開発段階
　・開発段階、チャネル
　　以下の段階となる。
　　・正式リリース前
...
　　　⋆Final Development Release(FDR)/γ:残りのほぼすべての機能を実装する。
　　　⋆Release Candidate(RC)/δ:カーネル決定、正式名称決定、最終調整など。
　　・正式リリース後(チャネル含む)
  　　⋆Canary:バージョンを指定しないα以前の開発版。
  　　⋆α:基本的な部分を実装する。
　　　⋆β:ほぼすべての部分を実装する。
　　　⋆Release Candidate(RC):複数のタイプを用意し、それを選別した後、次のRC2以降となる。ただし、1.x系とマイナーバージョンは選別をスキップする予定。
　　　⋆Stable:完成した安定板。
　　　⋆
　　・開発スケジュール(予定)
　　　⋆コミュニティ創設宣言:2025年8月29日
　　　⋆V0.1α開発開始:2026年3月18日~2026年4月22日
　　　⋆V0.1αリリース:2026年6月12日
...
　  また、開発者向けの内部コードネームとしてスイーツの名前が付いたコードネーム(主にアイスクリームやスイーツの名前)も決定する。ただ、マイナーバージョンは内部コードネームは制定しない可能性がある。
　　・決定したコードネーム
